<div class="filter dont-print">
    <div class="row">
        <div class="col-sm-4">
            ...
            <select name="course_id" id="course_id" class="js-example-basic-single" style="width:100% !important"
                autocomplete="off">
                <option value="">Select</option>
                @foreach ($all_courses as $key => $all_course)
                    <option value="{{ $all_course->id }}" @if (request()->get('course_id') == $all_course->id) selected @endif>
                        {{ $all_course->name }} ({{ $all_course->code }})</option>
                @endforeach
            </select>
        </div>
