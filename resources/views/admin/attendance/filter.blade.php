<div class="filter dont-print">
 <div class="row">
  <div class="col-sm-4">
   ...
   <select name="course_id" id="course_id" class="form-control js-example-basic-single" style="width:100% !important"
    autocomplete="off">
    <option value="">All</option>
    @if(auth('admin')->check())
    @foreach($courses as $key=>$course)
    <option value="{{$course->id}}" @if(request()->get("course_id") == $course->id) selected @endif >{{$course->name}}
     ({{$course->code}})</option>
    @endforeach
    @else
  ...
  <div class="col-sm-4">
   ...
   <select name="merit_master_id" id="merit_master_id" class="form-control" style="width:100% !important"
    autocomplete="off">
  ...
  <div class="col-sm-4">
   ...
   <input type="text" name="application_no" id="application_no" class="form-control input-sm" value="{{request()->get("
    application_no")}}" autocomplete="off">
  </div>
 </div>
 <div class="row">
  <div class="col-sm-4">
   <label for="status" class="label-control">Category</label>
   <select name="admission_category_id" id="admission_category_id" class="form-control" style="width:100% !important"
    autocomplete="off">
    <option value="">All</option>
    @foreach($admission_categories as $key=>$admission_category)
    <option value="{{$admission_category->id}}" {{request()->get("admission_category_id") == $admission_category->id ?
     "selected" : ''}}>{{$admission_category->name}}</option>
    @endforeach
   </select>
  </div>
  <div class="col-sm-4">
 ...
 <div class="row">
  <div class="col-sm-4">
   ...
   <select name="undertaking_status" id="undertaking_status" class="form-control" style="width:100% !important"
    autocomplete="off">
    <option value="">All</option>
    ...
    <option value="not_uploaded" {{request()->get("undertaking_status") == "not_uploaded" ? "selected" : ''}}>Not
     Uploaded</option>
   </select>
  </div>
  ...
  <div class="col-sm-4">
   ...
   <select name="payment_option" id="payment_option" class="form-control" style="width:100% !important"
    autocomplete="off">
    <option value="">All</option>
 ...
 <div class="row">
  <div class="col-sm-3">
   ...
   <a href="{{route('index')}}" class="btn btn-danger"><i class="fa fa-remove"></i> Reset</a>
  </div>
  {{-- <div class="col-sm-3">
