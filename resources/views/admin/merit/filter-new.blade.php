<div class="filter dont-print">
    <div class="row">
        ...
        <div class="col-sm-4">
            <label for="merit_master_id" class="label-control">Select List:</label>{{-- {{dd($list)}} --}}
    ...
    <div class="row">
        <div class="col-sm-4">
            ...
            <select name="application_no" class="js-example-basic-single" style="width:100% !important" autocomplete="off">
                <option value="">Select</option>
                @foreach($merit_lists_filter as $key=>$ml)
                <option value="{{$ml->application_no}}" @if($ml->application_no == request()->get("application_no")) selected @endif >{{$ml->application_no}}</option>
                @endforeach
            </select>
        </div>
        {{-- <div class="col-sm-4">
