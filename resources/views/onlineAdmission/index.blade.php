...
@section('content')
                        ...
                        <form {{-- action="{{route(get_route_guard().'.merit.automate')}}" method="post" --}}>
                            <div class="row">
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <select name="merit_master_id" id="merit_master_id" class="form-control"
                                        {{-- style="width:100% !important" --}} autocomplete="off">
                                        <option value="">--select--</option>
                                        @foreach ($list as $li)
                                            <option value="{{ $li->id }}"
                                                {{ request()->get('merit_master_id') == $li->id ? 'selected' : '' }}>
                                                {{ $li->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-sm-4">
                                    ...
                                    <select name="admission_cat" id="admission_cat" class="form-control">
                                        ...
                                        @foreach ($admission_cat as $cat)
                                            <option value="{{ $cat->admissionCategory->id }}"
                                                {{ request()->get('admission_cat') == $cat->admissionCategory->id ? 'selected' : '' }}>
                                ...
                                <div class="col-sm-3">
                                    @if (auth('admin')->check())
                                        <input type="submit" class="btn btn-warning" name="submit" value="Process"
                                            onclick="return confirm('Are you sure you want to Process? Hence it is Department user task, you can continue with search.');">
                                    @else
                                        <input type="submit" class="btn btn-warning" name="submit" value="Process">
                                    @endif
            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-body">
                        ...
                        <form name="merit" id="merit"
                            action="{{ route(get_route_guard() . '.merit.call-for-admission') }}" method="POST">
                            {{ csrf_field() }}
                                        ...
                                        <tr>
                                            <td>{{ ++$key }}
                                                @if ($list->new_status == 'can_call')
                                                    <input class="form-check-input" type="checkbox" name="merit_list_ids[]"
                                                        value="{{ $list->id }}">
                                                @endif
                                            </td>
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="row">
                                                <div class="col-md-8">
                                                    <label for="">Select Time Window for student to
                                                        complete</label>
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <input type="submit" value="Call For Admission"
                                                class="ladda-button btn btn-primary"
                                                onclick="return confirm('Are you sure you want Call Please check again?')"
    ...
    <div class="modal fade" id="cancelModal" tabindex="-1" role="dialog" aria-labelledby="cancelModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form
                    action="{{ route(get_route_guard() . '.merit.cancel-for-admission' /* ,Crypt::encrypt($list->id) */) }}"
                    method="post" id="cancel_candidate">
                    {{ csrf_field() }}
                    <input type="hidden" name="merit_list_id" class="merit_list_id" value="">
                    <div class="modal-header bg-danger">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-12">
                                <label for="">Reason Of Cancel <span style="font-size:11px; color:red;">( *
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <label for="">Email to candidate <span style="font-size:11px; color:red;">(If left
                                        blank, no email will be sent to the candidate)</span></label>
                                <textarea name="message" id="" cols="5" rows="3" class="form-control"
                                    onkeydown="restrictQuotes(event)" onkeyup="restrictQuotes(event)"></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        ...
                        <input type="submit" class="ladda-button btn btn-primary"
                            onclick="confirm('Are you sure you want to proceed with the operation? This cancellation cannot be rolled back.')"
                            value="Submit & Cancel" data-style="expand-right">
                    </div>
                </form>
            </div>
        </div>
    ...
    @if (auth('admin')->check())
        <div class="modal fade" id="newTime" tabindex="-1" role="dialog" aria-labelledby="newTimeLabel"
            aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action="{{ route(get_route_guard() . '.merit.assign-new-time' /* ,Crypt::encrypt($list->id) */) }}"
                        method="post">
                        {{ csrf_field() }}
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <input type="text" name="new_date_from" class="form-control datenew"
                                        autocomplete="off" required>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <input type="text" name="new_date_to" class="form-control datenew"
                                        autocomplete="off" required>
                                </div>
                            </div>
                        </div>
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('#cancel_candidate').on('submit', function(event) {
                laddaButton.start();
