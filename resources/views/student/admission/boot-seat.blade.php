...
@php
    function btechDocumentsCleared($merit){
        ...
        if(in_array($merit->course_id, btechCourseIds())){
            ...
            if($merit->approved_marksheet && ($merit->approved_prc || (!$merit->approved_prc && !$merit->active_prc && !$merit->rejected_prc)) && ($merit->approved_category || (!$merit->approved_category && !$merit->active_category && !$merit->rejected_category))){
                return true;
            }
            return false;
        }
    ...
    function btechNeedtoShowUploadButton($merit){
        ...
        if(in_array($merit->course_id, btechCourseIds())){
            // only check if b.tech candidates
            // marksheet is compulsory
            if((!$merit->approved_marksheet && !$merit->active_marksheet) || ($merit->rejected_prc) || ($merit->rejected_category)){
                return true;
            }
            return false;
        }
...
<div class="container">
    @if(checkPermission(2)==true)
		<div class="row">
            <a href="{{route('department.merit.index',['course_id'=>$course_id,'merit_master_id'=>$merit_master_id])}}" class="btn btn-primary">Go To Admission Officer Panel</a>
		</div>
    @endif
    @if(checkPermission(3)==true)
		<div class="row">
            <a href="{{route('department.merit.payment',['course_id'=>$course_id,'merit_master_id'=>$merit_master_id])}}" class="btn btn-primary">Go To Payment Officer Panel</a>
		</div>
    @endif
    <br/>
    <div class="row">
                                ...
                                @foreach ($application->merit_list as $key => $merit)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <br/><b style="color: rgb(150, 35, 35)">{{$merit->freezing_floating}}</b>
                                            @endif
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            @elseif($merit->status == 9)
                                                <span class="label label-danger"> Seat declined by {{$merit->seat_declined_by}}. </span>
                                            ...
                                            @if ($undertakings->count())
                                                ...
                                                <button class="btn btn-xs btn-danger" onClick="showUndertakings(this)" data-url="{{route("student.merit.undertaking-view", $merit->id)}}">
                                        ...
                                        @if(checkPermission(2)==true && in_array($merit->course_id, $programs))
                                                @if ($merit->status == 9)
                                                    <a target="_blank" href="{{route(get_route_guard().".merit.decline-receipt", Crypt::encrypt($merit->id))}}" class="btn btn-danger btn-xs">
                                                    ...
                                                    <div class="modal-content">
                                                        ...
                                                        <form action="{{ route(get_route_guard().'.merit.checklist', Crypt::encrypt($merit->id)) }}" method="post">
                                                                ...
                                                                <div class='col-md-1' style="align:left">
                                                                    @php
                                                                        ...
                                                                        $checked_data=DB::table('admission_checked_checklists')
                                                                                        ->where('merit_list_id',$application->merit_list[$index]->id)
                                                            ...
                                                            <div class="row ">
                                                                ...
                                                                <div class="col-md-10">
                                                                <textarea class="form-control" name="checklist_remarks" id="checklist_remarks" cols="30" rows="3" placeholder="Enter Your Coment" required>{!!$merit->checklist_remarks!!}</textarea><br/>
                                                                </div>
                                                            </div>
                                                            <div class="row">
                                                                <div class="col-md-5">
...
{{-- @php
    $checked_data=DB::table('admission_checked_checklists')->where('merit_list_id',$application->merit_list[0]->id)->get();
@endphp --}}
            ...
            <form action="{{ route(get_route_guard().'.merit.checklist', Crypt::encrypt($merit->id)) }}" method="post">
                ...
                <div class="row">
                    <div class='col-md-1' style="align:left">
                        @php
                            $checked_data=DB::table('admission_checked_checklists')
                                            ->where('merit_list_id',$application->merit_list[0]->id)
                                            ->where('document_id',$check->id)
                                            ->first();
                        @endphp
                        @if($checked_data==null)
                        {{++$key}} . <input type="checkbox" name="document[]" value="{{$check->id}}">
                        @else
                        {{++$key}} . <input type="checkbox" name="document[]" value="{{$check->id}}" checked>
                        @endif
                    </div>
                    <div class="col-md-10">
                        <label for="">  {{$check->name}}</label>
                ...
                <div class="row ">
                    ...
                    <div class="col-md-10">
                    <textarea class="form-control" name="checklist_remarks" id="checklist_remarks" cols="30" rows="3" placeholder="Enter Your Coment" required>{!!$application->merit_list[0]->checklist_remarks!!}</textarea><br/>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-5">
                        <input type="hidden" name="application_id" value="{{$application->merit_list[0]->application_no}}">
                        <input type="hidden" name="student_id" value="{{$application->merit_list[0]->student_id}}">
                        <input type="hidden" name="merit_id" value="{{$application->merit_list[0]->id}}">
                    </div>
                    <div class="col-md-4">
                        <input type="submit" class="btn btn-primary" value="Submit">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
