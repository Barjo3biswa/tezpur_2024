...
<table class="table table-bordered"   width="100%" border="1" style="border-collapse: collapse">
    <tbody>
        ...
        <tr>
            ...
            <td class="padding-xs bold">{{$application->nationality ?? "NA"}}</td>
        </tr>
        <tr>
            <td class="padding-xs">Aadhaar No</td>
            <td class="padding-xs bold">{{$application->adhaar ?? "NA"}}</td>
            <td class="padding-xs">DigiLocker ID<br/>About Academic Bank Of Credits(ABC) </td>
            <td class="padding-xs bold">{{$application->nad_id ?? "NA"}}<br/>{{$application->abc ?? "NA"}}</td>
        </tr>
        <tr>
            <td class="padding-xs">Category</td>
            <td class="padding-xs bold">{{$application->caste->name ?? "NA"}}</td>
            <td class="padding-xs">Sub-Caste</td>
            <td class="padding-xs bold">
                {{$application->sub_caste ?? "NA"}}
            </td>
        </tr>
        <tr>
            ...
            <td class="padding-xs bold">{{$application->place_residence ?? "NA"}}</td>
        </tr>
        <tr>
            <td class="padding-xs">Marital Status</td>
            <td class="padding-xs bold">{{$application->marital_status ?? "NA"}}</td>
            <td class="padding-xs">Country of origin</td>
            <td class="padding-xs bold">{{$application->student->country->name}}</td>                                    
        </tr>
        @if($application->is_foreign)
        <tr>
            <td class="padding-xs">Passport No</td>
            <td class="padding-xs bold">{{$application->passport_number ?? "NA"}}</td>
            <td class="padding-xs">Driving license or equivalent no</td>
            <td class="padding-xs bold">{{$application->driving_license_equivalnet_no ?? "NA"}}</td>                                    
        </tr>
        @endif
        <tr>                                    
            <td class="padding-xs">Are you person with disability (PWD) ?</td>
            <td class="padding-xs bold" colspan="3">
                {{$application->is_pwd==0 ? 'No': 'Yes'}}
                {{$application->person_with_disablity ? " (&nbsp;&nbsp;Type:".$application->person_with_disablity.")" : ""}}
                {{$application->pwd_percentage ? " (&nbsp;&nbsp;PWD Percentage:".$application->pwd_percentage.")" : ""}}
            </td>
        </tr>                                
        <tr>
            <td class="padding-xs">Are you a Kashmiri Migrant (KM)?</td>
            <td class="padding-xs bold">{{$application->is_kmigrant ? "Yes" : "No"}}</td>
            <td class="padding-xs">Are you a student from J & K?</td>
            <td class="padding-xs bold">
                {{$application->is_jk_student ? 'Yes':'No'}}
            </td>
        </tr>
        <tr>
            <td class="padding-xs">Are you ex-Serviceman or widow/ward<br> of ex-serviceman?</td>
            <td class="padding-xs bold" colspan="3">{{$application->is_ex_servicement ? "Yes" : "No"}} {{$application->priority ? "(".$application->priority->name.")" : ""}}</td>
        </tr>
        <tr>
            <td class="padding-xs">Do you belong to BPL/AAY?</td>
            <td class="padding-xs bold">{{$application->is_bpl ? "Yes" : "No"}}</td>
            <td class="padding-xs">Do you belong to minority community?</td>
            <td class="padding-xs bold">{{$application->is_minority ? "Yes" : "No"}}</td>
        </tr>
        <tr>
            <td class="padding-xs">Are you employed?</td>
            <td class="padding-xs bold" colspan="3">{{$application->is_employed ? "Yes" : "No"}} {{$application->is_employed ? "(".$application->employment_details.")" : ""}}</td>
        </tr>
        <tr>
            <td class="padding-xs">Have you represented the Country/State<br> in any sport?</td>
            <td class="padding-xs bold">
                {{$application->application_academic ? ($application->application_academic->is_sport_represented ? "Yes" : "No") : "No"}}
                {{$application->application_academic ? ($application->application_academic->sport_played ?  " (".$application->application_academic->sport_played.")" : "") : ""}}
            </td>
            <td class="padding-xs">Academic distinction/ medals/ prizes, if any</td>
            <td class="padding-xs bold">{{$application->application_academic ? ($application->application_academic->is_academic_prizes ? "Yes" : "No") : "No"}}</td>
        </tr>
        <tr>
            <td class="padding-xs">Other information worth mentioning including<br> publications, if any?</td>
            <td class="padding-xs bold" colspan="3">
                {{$application->application_academic->other_information ?? "NA"}}
            </td>
        </tr>
        <tr>
            <td class="padding-xs">Were you debarred from any examination (s)?</td>
            <td class="padding-xs bold">
                {{$application->application_academic ? ($application->application_academic->is_debarred ? "Yes" : "No") : "No"}}
            </td>
            <td class="padding-xs">Were you punished for misconduct?</td>
            <td class="padding-xs bold">
                {{$application->application_academic ? ($application->application_academic->is_punished ? "Yes" : "No") : "No"}}
                {{$application->application_academic ? ($application->application_academic->is_punished ? " (".$application->application_academic->furnish_details.")" : "") : ""}}
            </td>
        </tr>
        @if($application->applied_courses->first()->course->course_type_id == 4)
        <tr>
            <td class="padding-xs">JEE Main Roll No</td>
            <td class="padding-xs bold">{{$application->application_academic->jee_roll_no ??  "NA"}}</td>
            <td class="padding-xs">JEE Form No</td>
            <td class="padding-xs bold">{{$application->application_academic->jee_form_no ??  "NA"}}</td>
        </tr>
        <tr>
            <td class="padding-xs">JEE Year</td>
            <td class="padding-xs bold">{{$application->application_academic->jee_year ??  "NA"}}</td>
            <td colspan="2"></td>
        </tr>
        {{--  only PHD student--}}
        @elseif(in_array($application->applied_courses->first()->course->course_type_id, [11]))
        <tr>
            <td class="padding-xs">Whether passed/appeared the qualification exam?:</td>
            <td class="padding-xs bold">
                {{$application->application_academic->passed_or_appeared_qualified_exam ?? "NA"}}</td>
            <td class="padding-xs">Whether applying for full time/part time?:</td>
            <td class="padding-xs bold">
                @if($application->application_academic)
                    @if($application->application_academic->is_full_time && $application->application_academic->is_full_time)
                    Full Time
                    @else
                    Part time
                    @endif
                @else
                    NA
                @endif
                {{$application->academic_application->part_time_details ?? ""}}</td>
        </tr>
        <tr>
            <td class="padding-xs"> Experience related to:</td>
            <td class="padding-xs bold">{{$application->application_academic->academic_experience ?? "NA"}}</td>
            <td class="padding-xs">Publication Details (if any) :</td>
            <td class="padding-xs bold">{{$application->application_academic->publication_details ?? "NA"}}</td>
        </tr>
        <tr>
            <td class="padding-xs">Statement of Purpose (SOP) of research </td>
            <td class="padding-xs bold">{{$application->application_academic->statement_of_purpose ?? "NA"}}</td>
            <td class="padding-xs"></td>
            <td class="padding-xs bold"></td>
        </tr>
        @elseif(isMasterInDesign($application)) 
            <tr>
                <td class="padding-xs">CEED score </td>
                <td class="padding-xs bold">{{$application->application_academic->ceed_score ?? "NA"}}</td>
                <td class="padding-xs"></td>
                <td class="padding-xs bold"></td>
            </tr>
        @endif
        <tr>
            ...
            <td class="padding-xs bold">
                ...
                @if(getCorrespondencePermanentAddress($application))
                    C/O: {{$application->correspondence_co ?? "NA"}}</br>
                    House No:  {{$application->correspondence_house_no ?? "NA"}} </br> 
                    Street Name / Locality:  {{$application->correspondence_street_locality ?? "NA"}} </br> 
                    Vill/Town: {{$application->correspondence_village_town ?? "NA"}}</br>
                    {{-- PS: {{$application->correspondence_ps ?? "NA"}} </br> --}}
                    PO: {{$application->correspondence_po ?? "NA"}} </br>
                    Dist: {{$application->cor_district->district_name ?? $application->correspondence_district}} </br>
                    State: {{$application->cor_state->name ?? $application->correspondence_state}} - {{$application->correspondence_pin ?? "NA"}}
                @else
                    NA
                @endif
            </td>
            ...
            <td class="padding-xs bold">
                @if(getApplicationPermanentAddress($application))
                    ...
                    C/O:  {{$application->permanent_co ?? "NA"}}</br>
                    House No:  {{$application->permanent_house_no ?? "NA"}} </br> 
                    Street Name / Locality:  {{$application->permanent_street_locality ?? "NA"}} </br> 
                    Vill/Town:  {{$application->permanent_village_town ?? "NA"}} </br> 
                    {{-- PS: {{$application->permanent_ps ?? "NA"}} </br> --}}
                    PO: {{$application->permanent_po ?? "NA"}} </br>
                    Dist: {{$application->per_district->district_name ?? $application->permanent_district}} </br>
                    State: {{$application->per_state->name ?? $application->permanent_state }} -  {{$application->permanent_pin ?? "NA" }}
                @else
                    NA
                @endif
            </td>
    ...
    <table class="table table-bordered"   width="100%" border="1" style="border-collapse: collapse">
        ...
        <tr>
            ...
            <td class="padding-xs bold">{{$application->guardian_name ?? "NA"}}</td>
            <td class="padding-xs">Occupation</td>
            <td class="padding-xs bold">{{$application->guardian_occupation ?? "NA"}}</td>
        </tr>
        <tr>
            <td class="padding-xs">Mobile No</td>
            <td class="padding-xs bold">{{$application->guardian_phone ?? "NA"}}</td>
            <td class="padding-xs">Email</td>
            <td class="padding-xs bold">{{$application->guardian_email ?? "NA"}}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td class="padding-xs bold" colspan=3>{{$application->fatherQualification->name ?? "NA"}}</td>
        </tr>
        <tr>
            <td class="padding-xs">Mobile No</td>
            <td class="padding-xs bold">{{$application->father_mobile ?? "NA"}}</td>
            <td class="padding-xs">Email</td>
            <td class="padding-xs bold">{{$application->father_email ?? "NA"}}</td>
        </tr>
       {{--  <tr>
            <td class="padding-xs">Annual Income</td>
            <td class="padding-xs bold">{{$application->father_income_range ? $application->father_income_range->min."-".$application->father_income_range->max: "NA"}}</td>
            <td colspan="2"></td>
        </tr> --}}
        ...
        <tr>
            ...
            <td class="padding-xs bold" colspan=3>{{$application->motherQualification->name ?? "NA"}}</td>
        </tr>
        <tr>
            <td class="padding-xs">Mobile No</td>
            <td class="padding-xs bold">{{$application->mother_mobile ?? "NA"}}</td>
            <td class="padding-xs">Email</td>
            <td class="padding-xs bold">{{$application->mother_email ?? "NA"}}</td>
        </tr>
       {{-- <tr>
            <td class="padding-xs">Annual Income</td>
            <td class="padding-xs bold">{{$application->mother_income_range ? $application->mother_income_range->min."-".$application->mother_income_range->max: "NA"}}</td>
            <td colspan="2"></td>
        </tr>--}}
        <tr>
            <td class="padding-xs">Family  Income</td>
            <td class="padding-xs bold">{{$application->family_income_range ? ($application->family_income_range->min."-".$application->family_income_range->max): "NA"}}</td>
            <td class="padding-xs">PAN No</td>
            <td class="padding-xs bold">{{$application->application_academic ? ($application->application_academic->pan_no ?? "NA") : "NA"}}</td>
...
<table class="table table-bordered"   width="100%" border="1" style="border-collapse: collapse">
    <tbody>
        ...
        <tr>
            ...
            <td class="padding-xs bold">
               {{$application->bank_ac_no??"NA"}}
            </td>
            <td class="padding-xs ">Bank Name</td>
            <th class="padding-xs bold"> 
                {{$application->bank_name??"NA"}}
            </th>
        </tr>
        <tr>
            <td class="padding-xs ">Branch Name</td>
            <td class="padding-xs bold">{{$application->branch_name}}</td>
            <td class="padding-xs">IFSC CODE</td>
            <td class="padding-xs bold">{{$application->ifsc_code?? "NA"}}</td>
        </tr>
        
        <tr>
            <td class="padding-xs ">Mobile </td>
            <td class="padding-xs bold">{{$application->bank_reg_mobile_no?? "NA"}}</td>
            <td class="padding-xs ">A/C Holder Name </td>
            <td class="padding-xs bold">{{$application->account_holder_name?? "NA"}}</td>
...
@if(/* $application->is_mba!=1 && */ $application->exam_through=='CUET')
...
<table class="table table-bordered"   width="100%" border="1" style="border-collapse: collapse">
    ...
    <tbody>
        <tr>
            <td>{{$academics->cuet_roll_no}}</td>
            <td>{{$academics->cuet_form_no}}</td>
            <td>{{$academics->cuet_year}}</td>
        </tr>
    </tbody>
</table>
...
<table class="table table-bordered"   width="100%" border="1" style="border-collapse: collapse">
    ...
    <tr>
       <th>sl No</th>
       <th>Subject name</th>
       <th>CUET Normalised Score(In Figures) </th>
