...
class newAdmissionController extends Controller
{
    ...
    public function semiAutomateProcess($admission_cat, $merit_master_id)
    {
        ...
        if ($course_seat->admission_flag == "open") {
            if ($admission_cat == 7) {
                $check_count = MeritList::where('is_pwd', 1)->where('merit_master_id', $merit_master_id)->whereIn('new_status', ['can_call', 'called','time_extended'])->count();
            } else {
    ...
    public function automateProcess($admission_cat, $merit_master_id)
    {
        ...
        $processing_student=MeritList::where('course_id', $merit_master->course_id)
                                    ...
                                    ->where('admission_category_id', $admission_cat)
                                    ->whereIn('new_status', ['can_call','called','time_extended'])->count();
        ...
        try {
            ...
            if(($available_seat + $course_seat->temp_seat_applied + $course_seat->total_seats_applied)>$course_seat->total_seats){
    ...
    public function callForAdmission(Request $request)
    {
        ...
        $course_seat_validate  = CourseSeats::where('course_id', $course_id_validate)
                                    ->where('admission_category_id', $ml_validate->admission_category_id)
                                    ->where('course_seat_type_id', $merit_master_validate->course_seat_type_id)->first();
